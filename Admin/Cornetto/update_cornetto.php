...
if (!empty($_POST))
{
    ...
    if (($isSucces && $isImageUpdated && $isUploadSucces) || ($isSucces &&!$isImageUpdated)){
        ...
        if ($isImageUpdated)
        {
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ?,  image= ? WHERE id = ?  restau_id = 5");
            $statement->execute(array($nom,$description,$price,$category,$image,$id));
        }
        else{
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ? WHERE id = ? and restau_id = 5");
            $statement->execute(array($nom,$description,$price,$category,$id));
        }
        Database::disconnect();
        header("Location: index_cornetto.php");
    }
    else if ($isImageUpdated && !$isUploadSucces)
    {
        $db =Database::connect();
        $statement = $db->prepare("SELECT image FROM repas  WHERE id = ? and restau_id = 5");
...
{
    $db =Database::connect();
    $statement = $db->prepare("SELECT * FROM repas  WHERE id = ? and restau_id = 5");
    $statement->execute(array($id));
    $repas = $statement->fetch();
    $nom        = $repas['nom'];
...
?>
    ...
    <div class="row">
        ...
        <br>
        <form class="form" role="form" action="<?php echo 'update_cornetto.php?id=' . $id; ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="category_id" name="category_id">
                    <?php
                    Database::connect();
                    foreach ($db->query('SELECT * FROM menu WHERE id in (1,2,3,4,6,8,9,10,11)') as $row)
                    {
                        if ($row['id'] == $category)
                        {
            ...
            <div class="form-action">
                ...
                <a class="btn btn-primary" href="index_cornetto.php"
