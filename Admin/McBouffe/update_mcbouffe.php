...
if (!empty($_POST))
{
    ...
    if (($isSucces && $isImageUpdated && $isUploadSucces) || ($isSucces &&!$isImageUpdated)){
        ...
        if ($isImageUpdated)
        {
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ?,  image= ? WHERE id = ?  restau_id = 2");
            $statement->execute(array($nom,$description,$price,$category,$image,$id));
        }
        else{
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ? WHERE id = ? and restau_id = 2");
    ...
    {
        $db =Database::connect();
        $statement = $db->prepare("SELECT image FROM repas  WHERE id = ? and restau_id = 2");
...
{
    $db =Database::connect();
    $statement = $db->prepare("SELECT * FROM repas  WHERE id = ? and restau_id = 2");
    $statement->execute(array($id));
    $repas = $statement->fetch();
    $nom        = $repas['nom'];
...
?>
    ...
    <div class="row">
        ...
        <br>
        <form class="form" role="form" action="<?php echo 'update_mcbouffe.php?id=' . $id; ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="category_id" name="category_id">
                    <?php
                    Database::connect();
                    foreach ($db->query('SELECT * FROM menu WHERE id in (1,2,6,7,8,10)') as $row)
                    {
                        if ($row['id'] == $category)
                        {
            ...
            <div class="form-action">
                <button type="submit" class="btn btn-success"
                 data-bs-toggle="tooltip" data-bs-placement="top" title="Modifier un  repas"><span class="glyphicon glyphicon-pencil"></span> Modifier</button>
                <a class="btn btn-primary" href="index_mcbouffe.php"
