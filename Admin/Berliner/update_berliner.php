...
if (!empty($_POST))
{
    ...
    if (($isSucces && $isImageUpdated && $isUploadSucces) || ($isSucces &&!$isImageUpdated)){
        ...
        if ($isImageUpdated)
        {
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ?,  image= ? WHERE id = ?  restau_id = 4");
            $statement->execute(array($nom,$description,$price,$category,$image,$id));
        }
        else{
            $statement = $db->prepare("UPDATE repas set nom= ?,  description= ?,  price= ?,  category_id= ? WHERE id = ? and restau_id = 4");
    ...
    {
        $db =Database::connect();
        $statement = $db->prepare("SELECT image FROM repas  WHERE id = ? and restau_id = 4");
...
{
    $db =Database::connect();
    $statement = $db->prepare("SELECT * FROM repas  WHERE id = ? and restau_id = 4");
    $statement->execute(array($id));
    $repas = $statement->fetch();
    $nom        = $repas['nom'];
...
?>
    <div class="row">
        ...
        <br>
        <form class="form" role="form" action="<?php echo 'update_berliner.php?id=' . $id; ?>" method="post" enctype="multipart/form-data">
