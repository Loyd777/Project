<title>Mcbouffe</title>
<?php
include "header.php";
echo '<nav>
               <ul class="nav nav-pills">';
$db = Database::connect();
$statement = $db->query('SELECT * FROM menu WHERE id in (1,2,6,7,8,10) ');
...
foreach ($menu as $category){
    ...
    echo '<div class="row">';
    $statement = $db->prepare('SELECT * FROM repas WHERE `category_id`= ? AND `restau_id`= 2;');
